...
A destination contract can validate a callback by checking that the sender is the callback proxy (so the call is coming through the expected entry point).

:::info[Hyperlane]
Some networks can’t act as destination chains yet because a callback proxy hasn’t been deployed there. In that case, use [Hyperlane](./hyperlane) as the transport for cross-chain callbacks.
:::
