...
A callback contract lives on a destination chain and receives calls triggered by a reactive contract. When a [reactive contract](./reactive-contracts.mdx) requests a callback, the network submits a transaction on the destination chain through a callback proxy, and that proxy calls your contract. From your contract's side, nothing exotic happens: it's an ordinary function call from an ordinary address. There's no `react()`, no subscription, and no `LogRecord`. What a callback contract needs is a way to prove the call is authentic, and funds to pay for it.
