...
- Where `react()` sits in the wider flow, from the debt check that precedes it to the charge that follows, is laid out in [Economy](./economy.md).
...
Leaving it open to any caller lets a third party fund the contract and restart it, which is useful for contracts serving other people. Add `onlyOwner` if the balance is meant for anything else, or omit the wrapper entirely and rely on `depositTo()`. [Economy](./economy.md) covers the trade-offs and the commands.
