...
- **Keep the contract funded.** Reactive transactions cost gas and the contract pays for them. A `payable` constructor lets you fund at deployment, which is the simplest route, but it isn't required: `receive()` is inherited as payable, so a contract can be topped up afterwards, and `depositTo()` credits it without its cooperation. What matters is that funds are there before the first event arrives. [Economy](./economy.mdx) covers reserves, charges, and debt.
...
A subscription tells Reactive which logs should reach your contract. Each one is a set of criteria passed to `SYSTEM.subscribe()`, and both methods take the same six arguments:
...
Each criterion narrows or widens independently. `topic0` is normally the event signature hash, with topics 1-3 holding indexed parameters, and any position can be left open:
...
- Where `react()` sits in the wider flow, from the debt check that precedes it to the charge that follows, is laid out in [Economy](./economy.mdx).
...
- Callbacks are billed to the recipient on the destination chain, in that chain's native currency, not to the reactive contract that requested them. A recipient in debt receives nothing, and a failed callback still costs it. See [Callback Contracts](./callback-contracts.mdx).
...
Writing the contract that receives the callback is covered in [Callback Contracts](./callback-contracts.md).

## Settling Debt On Demand

A contract in debt receives nothing: the system contract stops delivering reactive transactions until the debt clears. Charges draw on reserves first and [AbstractPayer](https://github.com/Reactive-Network/reactive-lib-omni/blob/master/src/base/AbstractPayer.sol) settles any shortfall automatically through `pay()`, so this only bites a contract that has run out of both.

`_coverDebt()` settles from the contract's own balance, but it's internal, so expose it yourself if an outside call should be able to trigger it:
...
Leaving it open to any caller lets a third party fund the contract and restart it, which is useful for contracts serving other people. Add `onlyOwner` if the balance is meant for anything else, or omit the wrapper entirely and rely on `depositTo()`. [Economy](./economy.mdx) covers the trade-offs and the commands.
